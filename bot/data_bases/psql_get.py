...
class DB_otw(DB_conn):
    ...
    async def change_col(self, col, name):
        try:
            ...
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    await conn.execute(f"UPDATE administration_item SET col=col-{col} WHERE name='{name}';")
        except:
            print("MDA")
    ...
    async def get_row_by_name(self, name):
        try:
            ...
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    row = await conn.fetchrow(f"SELECT * FROM administration_item WHERE name='{name}';")
        except:
            print("MDA")
        else:
            return row
